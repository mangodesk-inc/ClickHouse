...
{
    LOG_DEBUG(getLogger("RestoreChuns"), "Restoring chunk infos, before: {}, restoring: {}",
        chunk.getChunkInfos().debug(), chunk_infos.debug());

    chunk.getChunkInfos().appendIfUniq(chunk_infos.clone());

    LOG_DEBUG(getLogger("RestoreChuns"), "Restoring chunk infos, result: {}",
